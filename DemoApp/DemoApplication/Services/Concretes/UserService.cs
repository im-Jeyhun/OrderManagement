...
namespace DemoApplication.Services.Concretes
{
    public class UserService : IUserService
    {
        ...
        public async Task CreateAsync(RegisterViewModel model)
        {
            ...
            var contentLink = GenerateUrl<Guid>("client-activated", new { token = token });
        ...
        {
            var message = new Message(user.Email, title, content);
            _emailService.Send(message);
        }

        public string GenerateUrl<T> (string rouuteName , object routeParams )
        {
            var request = _httpContextAccessor.HttpContext.Request;

            var url = _urlHelper.RouteUrl(rouuteName, routeParams, request.Scheme, request.Host.Value)!;
            //request schema burada domain , request host value port olur 
            return _urlHelper.RouteUrl(rouuteName, routeParams, request.Scheme , request.Host.Value)!;
        }
    }
}
